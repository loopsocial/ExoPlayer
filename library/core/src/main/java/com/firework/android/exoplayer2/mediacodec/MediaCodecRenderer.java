...
import com.firework.android.exoplayer2.drm.FrameworkCryptoConfig;
...
import com.firework.android.exoplayer2.decoder.CryptoConfig;
import com.firework.android.exoplayer2.decoder.DecoderInputBuffer;
import com.firework.android.exoplayer2.decoder.DecoderInputBuffer.InsufficientCapacityException;
...
import com.firework.android.exoplayer2.util.NalUnitUtil;
import com.firework.android.exoplayer2.util.TimedValueQueue;
...
public abstract class MediaCodecRenderer extends BaseRenderer {
  ...
  public static class DecoderInitializationException extends Exception {
    ...
    private static String buildCustomDiagnosticInfo(int errorCode) {
      ...
      return "com.firework.android.exoplayer2.mediacodec.MediaCodecRenderer_"
          + sign
          + Math.abs(errorCode);
    }
  }
  ...
  public void render(long positionUs, long elapsedRealtimeUs) throws ExoPlaybackException {
    ...
    try {
      ...
      if (inputFormat == null && !readSourceOmittingSampleData(SampleStream.FLAG_REQUIRE_FORMAT)) {
      ...
      if (bypassEnabled) {
        ...
        readSourceOmittingSampleData(SampleStream.FLAG_PEEK);
      }
   ...
   *
   * @param readFlags Additional {@link SampleStream.ReadFlags}. {@link SampleStream#FLAG_OMIT_SAMPLE_DATA} is
  ...
  private boolean readSourceOmittingSampleData(@SampleStream.ReadFlags int readFlags)
    ...
    int result = readSource(formatHolder, noDataBuffer, readFlags | SampleStream.FLAG_OMIT_SAMPLE_DATA);
  ...
  protected DecoderReuseEvaluation onInputFormatChanged(FormatHolder formatHolder)
    ...
    @DecoderReuseEvaluation.DecoderDiscardReasons int overridingDiscardReasons = 0;
    switch (evaluation.result) {
      case DecoderReuseEvaluation.REUSE_RESULT_NO:
        drainAndReinitializeCodec();
        break;
      case DecoderReuseEvaluation.REUSE_RESULT_YES_WITH_FLUSH:
        if (!updateCodecOperatingRate(newFormat)) {
          overridingDiscardReasons |= DecoderReuseEvaluation.DISCARD_REASON_OPERATING_RATE_CHANGED;
        } else {
          codecInputFormat = newFormat;
          if (drainAndUpdateCodecDrmSession) {
            if (!drainAndUpdateCodecDrmSessionV23()) {
              overridingDiscardReasons |= DecoderReuseEvaluation.DISCARD_REASON_WORKAROUND;
            }
    ...
    if (evaluation.result != DecoderReuseEvaluation.REUSE_RESULT_NO
      ...
      return new DecoderReuseEvaluation(
          codecInfo.name, oldFormat, newFormat, DecoderReuseEvaluation.REUSE_RESULT_NO, overridingDiscardReasons);
    }
  ...
  private void bypassRead() throws ExoPlaybackException {
    ...
    while (true) {
      bypassSampleBuffer.clear();
      @SampleStream.ReadDataResult int result = readSource(formatHolder, bypassSampleBuffer, /* readFlags= */ 0);
      switch (result) {
